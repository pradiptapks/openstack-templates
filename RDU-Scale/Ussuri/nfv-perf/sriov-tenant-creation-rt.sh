...
nfv_provider_nw () {	
	...
	run_cmd "openstack subnet create --network managament --gateway 192.168.176.1 --subnet-range 192.168.176.0/24 managament-subnet";
...
nfv_ports () {
	sec_fun;
	openstack port create --network provider-1 --no-security-group --disable-port-security --vnic-type direct --binding-profile trusted=true provider1-port1;
	openstack port create --network provider-2 --no-security-group --disable-port-security --vnic-type direct --binding-profile trusted=true provider2-port1;
