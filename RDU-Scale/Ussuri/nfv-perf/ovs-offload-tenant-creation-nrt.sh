...
nfv_flavor () {
	...
	run_cmd "openstack flavor create nfv-large --id auto --ram 20480 --disk 30 --vcpus 8 --property hw:numa_nodes=1 --property hw:cpu_policy='dedicated' --property hw:mem_page_size='1GB' --property hw:emulator_threads_policy='isolate' --property hw:isolated_metadata='true' --property hw:pmu='false'";
...
nfv_instance () {
	...
	run_cmd "openstack server create --flavor nfv-large --security-group $security --nic port-id=$managament_port1 --nic port-id=$provider1_port1 --nic port-id=$provider2_port1 --key-name key1 --image rhel8.2 --availability-zone nfvprovider TestPMD --wait";
	}
